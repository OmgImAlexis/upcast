...
(function () {
	...
	describe('upcast', () => {
		...
		describe('.is()', () => {
			...
			it('should utilise the type function', () => {
				upcast.is('foo', 'type1');
				assert.isTrue(upcast.type.calledOnce);
			});

			it('should return true when called with the correct type', () => {
				assert.isTrue(upcast.is('foo', 'type1'));
			});

			it('should return false when called with the incorrect type', () => {
				assert.isFalse(upcast.is('bar', 'type1'));
			});

			it('should throw when called with a non-string type', () => {
				assert.throws(() => {
					upcast.is('bar', 123);
		...
		describe('.to()', () => {
			...
			it('should guard against types without casters', () => {
